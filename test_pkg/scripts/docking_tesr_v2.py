...
class MinimalPublisher2(Node):
    ...
    def __init__(self):
        ...
        # read config file option 1
        file_path = os.path.join(os.path.dirname(__file__), "../config/docking_tesr2.yaml")

        # read config file option 2
        ...
        # print(self.yaw_adjust_disable_by_distance_x)
    ...
    def test2(self):
        try:
            ...
            if len(self.apriltag_msg.detections) == 3 and ((abs(self.tf_tag25_to_robot.transform.translation.x) >= self.min_dist_to_deatect_all_tags) or (self.first_point == True)):
                ...
                if self.first_point == False:
                    try:
                        if abs(self.tf_tag25_to_robot.transform.translation.x) < self.min_dist_to_deatect_all_tags: #robot close to station < 0.4, 0.5 / 0.45 meters // maximum distance to detect 3 tags
    ...
    def test3(self):
        ...
        try:
            ...
            # PID
            # if abs(self.tf_tag25_to_robot.transform.translation.x) > self.docking_distance + self.distance_goal_tolerance :
            if abs(self.tf_tag25_to_robot.transform.translation.x) > self.docking_distance + self.distance_goal_tolerance_last_predocking :
                ...
                if (math.fabs(distance_to_goal) > self.distance_goal_tolerance and self.reached_distance_goal == False ):
                    ...
                    # if (math.fabs(heading_error) > self.heading_error_threshold) and abs(self.tf_tag25_to_robot.transform.translation.x) > 0.5: # > 0.5, 0.6 ถ้าระยะน้อยกว่า x จะไม่ปรับ major heading  
                    if (math.fabs(heading_error) > self.heading_error_threshold) and abs(self.tf_tag25_to_robot.transform.translation.x) >= self.yaw_adjust_disable_by_distance_x:
                    # if (math.fabs(heading_error) > self.heading_error_threshold) :
                        ...
                        #option 3
                        if abs(self.tf_tag25_to_robot.transform.translation.x) <= self.yaw_adjust_disable_by_distance_x: # < 0.4, 0.45, 0.55 # ถ้าระยะน้อยกว่า x จะไม่ปรับ heading
                            self.get_logger().info("1.3")
